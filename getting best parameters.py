...
# now = filename.replace('./data/F2_dataset_add_features_model_', '').replace('.pkl', '')：filenameから日付部分を抽出して、対応するベストパラメータのJSONファイルのパスを生成します。このパスは、前のセクションでベストパラメータを保存したファイルのパスと一致する必要があります。
