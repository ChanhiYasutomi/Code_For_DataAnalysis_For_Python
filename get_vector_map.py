...
# 関数を使用してCategoryカラムのベクトルマップを作成
vector_map, dim = get_vector_map(df, 'Category')

# ベクトルマップの内容を確認
print(vector_map)
# 出力: {'A': [1, 0, 0], 'B': [0, 1, 0], 'C': [0, 0, 1]}

print(dim)
# 出力: 3

# この例では、get_vector_map 関数が Category カラムを対象にしています。このカラム内のカテゴリ（A、B、C）は、ユニークな値として抽出されます。
# それぞれのカテゴリに対して、次元数（3次元）と同じ長さのゼロベクトルを作成し、対応するカテゴリの次元を1に設定します。これにより、カテゴリカルなデータを数値のベクトルに変換することができます。
# この例では3つのカテゴリがあり、それぞれが3次元のベクトルに変換されています。ベクトルマップと次元数が関数から返されます。
